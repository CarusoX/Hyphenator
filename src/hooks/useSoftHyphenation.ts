...
const checkStringFitsOnBox = <T extends HTMLElement>(ref: React.RefObject<T>, str: string) => {
  if (!ref.current) return false;
  const backup = ref.current.innerText;
...
const checkStringFitsOnLine = <T extends HTMLElement>(ref: React.RefObject<T>, str: string) => {
  if (!ref.current) return false;
  const backup = ref.current.innerText;
...
const hyphenate = <T extends HTMLElement>(ref: React.RefObject<T>, str: string) => {
  ...
  for (let index = 0; index < chars.length; index = index + 0) {
    ...
    if (index === 0 || fits) {
      ...
      const fitsOnBox = checkStringFitsOnBox(ref, `${newHyphenatedContent}${char}`);
      const fitsOnBoxWithHyphen = checkStringFitsOnBox(ref, `${newHyphenatedContent}${char}-`);
...
export const useSoftHyphenation = <T extends HTMLElement>(ref: React.RefObject<T>, innerText: string) => {
  useLayoutEffect(() => {
